...
deprecate_colocated_module(path="geometry.test_shape", migrated_to="tests/geometry/ (pending)")
...
class TestShape(TestCase):
    ...
    def test_convert_units(self):
        ...
        self.assertAlmostEqual(
            self.s.size.magnitude,
            65.0 * ureg(units["length"]).to("ft").magnitude,
            places=3,
        )
...
class TestDatum(TestCase):
    ...
    def test_convert_units(self):
        ...
        self.assertAlmostEqual(
            self.d.elevation.magnitude,
            100.0 * ureg(units["length"]).to("ft").magnitude,
            places=3,
        )
